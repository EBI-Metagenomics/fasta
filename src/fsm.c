...
#define FASTA_MATCH_EXCESS_SIZE 5
...
static enum fasta_rc desc_begin(struct args *a)
{
    ...
    a->aux->end = a->aux->begin + FASTA_DESC_MAX;
    return read_desc(a);
}
...
static enum fasta_rc read_id(struct args *a)
{
    ...
    char const *ptr =
        memccpy(a->tgt->id, a->tok->value + 1, '\0', FASTA_ID_MAX);
    if (!ptr)
...
static enum fasta_rc seq_begin(struct args *a)
{
    ...
    a->aux->pos = a->aux->begin + 1;
    a->aux->end = a->aux->begin + FASTA_SEQ_MAX;
    return seq_cont(a);
}
...
static enum fasta_rc store_id(struct args *a)
{
    assert(a->tok->id == TOK_ID);
    char const *ptr =
        memccpy(a->aux->id, a->tok->value + 1, '\0', FASTA_ID_MAX);
    if (!ptr)
        return error_parse(a->tok->error, a->tok->line.number, "too long id");
    if (ptr - a->aux->id == 1)
        return error_parse(a->tok->error, a->tok->line.number, "empty id");
